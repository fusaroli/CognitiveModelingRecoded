...
df <- tibble(x1=x[,1], x2 = x[,2], se1=sigmaerror[1], se2=sigmaerror[2])

## Specify where the model is
file <- file.path(here("11_PearsonCorrelationMeasurement", "11_PearsonCorrelationMeasurement.stan"))
...
## Now in brms
m <- brm(
  bf(x1|se(se1, sigma = TRUE) ~ 1) + 
    bf(x2|se(se2, sigma = TRUE) ~ 1) + 
